...
public class Ball extends Gameobject {
    ...
    private void plateCollisionCheck() {
        if (pos_x < board.getRemotablePlate().pos_x && pos_x > board.getRemotablePlate().pos_x - 10 && pos_y < board.getRemotablePlate().pos_y + Const.PLATE_HEIGHT && pos_y + Const.PLATE_HEIGHT> board.getRemotablePlate().pos_y) {
            balldirx = -SPEED;
            Sound.play(SoundEffectType.ONHIT);
        } else if (pos_x < board.getComputerPlate().pos_x && pos_x > board.getComputerPlate().pos_x - 10 && pos_y < board.getComputerPlate().pos_y + Const.PLATE_HEIGHT && pos_y + Const.PLATE_HEIGHT > board.getComputerPlate().pos_y) {
            balldirx = SPEED;
            Sound.play(SoundEffectType.ONHIT);
        }
    }
